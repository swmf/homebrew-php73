class PhpAT7312 < Formula
  ...
  def install
    ...
    # Fix operator return type, refer to https://github.com/php/php-src/pull/7596

    inreplace "ext/intl/breakiterator/codepointiterator_internal.h" do |s|
      s.gsub! "virtual UBool operator==",
              "virtual bool operator=="   
    end

    inreplace "ext/intl/breakiterator/codepointiterator_internal.cpp" do |s|
      s.gsub! "UBool CodePointBreakIterator::operator==",
              "bool CodePointBreakIterator::operator=="
    end

    inreplace "Makefile" do |s|
      s.gsub! "apxs -S LIBEXECDIR='$(INSTALL_ROOT)/usr/local/Cellar/php@7.3.12",
              "apxs -S LIBEXECDIR='$(INSTALL_ROOT)/usr/local/Cellar/php\\@7.3.12"

    make_args = %W[
      EXTRA_CFLAGS=-DTRUE=1\ -DFALSE=0
      EXTRA_CXXFLAGS=-DTRUE=1\ -DFALSE=0
    ]
    
    system "make", *make_args
    p "^D to continue with make install"
    $stdin.read
    system "make", "install"
